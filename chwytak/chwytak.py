...
class GripperNode(Node):
    ...
    def __init__(self):
        ...
        try:
            ...
            self.pose_open_list: list[int] = [
                2 + degree / 18 for degree in self.get_parameter("pose_open_list").value
            ]
            ...
            # Pin numeration convention (BCM = GPIO numeration, BOARD = board's pin numeration)
            ...
            for pin in self.pin_list:
                ...
                self.servo[pin] = GPIO.PWM(pin, 50)
                self.servo[pin].start(0)
            ...
            for pose_open, pose_close, pin in zip(
                ...
                if self.state == "close" and self.old_state == "open":
                    for duty in range(pose_open, pose_close, step_up):
                        ...
                        self.servo[pin].ChangeDutyCycle(duty / scale)
                        time.sleep(0.001)
        ...
        try:
            ...
            while sum(servos_number) != 0:
                for i in range(len(servos_number)):
                    ...
                    self.servo[self.pin_list[i]].ChangeDutyCycle(current_pose[i])
    ...
    def __del__(self):
        for __servo in self.servo:
            self.get_logger().info(f"Cleaning up servo")
            __servo.stop()
